...
file_path = r"C:\Users\selin\OneDrive\Masaüstü\data_for _cough\29.csv"
...
fs = 4800
N = len(pulmonary_signal)
signal_centered = pulmonary_signal - np.mean(pulmonary_signal)
...
plt.yscale("log")  # küçük genlikleri gör
plt.title("Pulmoner Sinyal Frekans Spektrumu (Ort. Çıkartılmış)")
plt.xlabel("Frekans (Hz)")
plt.ylabel("Genlik (log)")
plt.grid(True)
plt.xlim(0, 3000)
plt.show()
...
# --- Kısa Süreli Yanlış Pozitifleri Silmek için Minimum Süre Kontrolü ---
min_duration_samples = int(0.9 * fs)  # 150 ms (istediğin süreye göre değiştir)

final_button_data_fixed = np.copy(final_button_data)

start = None
for i in range(len(final_button_data)):
    if final_button_data[i] == 1 and start is None:
        start = i
    elif (final_button_data[i] == 0 or i == len(final_button_data)-1) and start is not None:
        end = i
        duration = end - start
        if duration < min_duration_samples:
            final_button_data_fixed[start:end] = 0  # kısa segmentleri sıfırla
        start = None
...
for i in range(len(final_button_data_fixed)):
    current_button_state = final_button_data_fixed[i] # Mevcut butun değeri
